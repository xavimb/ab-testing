...
module.exports = {
    createTest: function (config) {
        ...
        var seed = Math.random().toString(36).substr(2, 7);

        var getRandomGroup = function (user) {
            if(user === undefined || typeof user !== 'string')
                throw Error("Undefined user to give it a random group");

            var hexMD5hash = crypto.createHash('md5').update(seed + user).digest('hex').substr(0, 8);
...
var initTest = function (config) {
    ...
    var tests = _.map(config, function (test) {
        if(test.name && typeof test.name === 'string') {
            return {
                // Weight from 0..1
                weight: (test.weight || defaultWeight) / totalWeight,
                name: test.name
            }
        }
    }).sort(function (a, b) {
